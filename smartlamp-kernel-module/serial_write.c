...
#define VENDOR_ID   0xXXXX  /* Substitua pelo VendorID do smartlamp */
#define PRODUCT_ID  0xYYYY  /* Substitua pelo ProductID do smartlamp */
...
static int  usb_write_serial(const char *cmd, int param);                          // Função para escrever na porta serial
...
static int usb_probe(struct usb_interface *interface, const struct usb_device_id *id) {
    ...
    // Testa o driver enviando os comandos de SET_LED
...
static int usb_write_serial(const char *cmd, int param) {
    ...
    // Formata o comando na usb_out_buffer
    snprintf(usb_out_buffer, usb_max_size, "%s %d", cmd, param);

    // Grave o valor de usb_out_buffer com printk para debug
